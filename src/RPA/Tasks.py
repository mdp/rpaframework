...
class Schema:
    ...
    def _parse_schema(self, schema, names):
        ...
        for name, properties in schema.get("tasks", {}).items():
            ...
            if properties.get("start", False):
                assert self.start is None, f"Duplicate start task"
                self.start = name
    ...
    def _create_action(self, action):
        ...
        assert "task" in action, f"Next task undefined for action"
    ...
    def validate(self, src, dst):
        ...
        if dst == "end":
            if self.end and src not in self.end:
                raise SchemaError(f"Unexpected end task")
            return
...
class Tasks:
    ...
    def _end_suite(self, data, result):
        ...
        if not self.graph_opts.pop("enabled", False):
            return
        ...
        try:
            path = self.graph.render(filename, dirname, **self.graph_opts)
